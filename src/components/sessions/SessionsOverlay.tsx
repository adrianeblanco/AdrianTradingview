...
import { sessionRangesInWindow, hexWithAlpha } from "@/lib/sessions";
...
export function SessionsOverlay({ chart, visibleRange }: Props) {
  ...
  const [bands, setBands] = useState<
    Array<{
      id: string; label: string; x1: number; x2: number;
      fill: string; stroke: string; emphasis?: boolean;
    }>
  >([]);
    ...
    const compute = () => {
      ...
      for (const r of ranges) {
        ...
        out.push({
          ...
          x1, x2,
          fill: hexWithAlpha(r.session.color, r.session.opacity),
          stroke: hexWithAlpha(r.session.color, r.session.borderOpacity),
    ...
    <svg className="pointer-events-none absolute inset-0 h-full w-full" style={{ zIndex: 2 }}>
      {bands.map(b => (
        <g key={b.id}>
          <rect
            x={b.x1} y={0}
            width={Math.max(b.x2 - b.x1, 1)} height="100%"
            fill={b.fill}
            stroke={b.stroke}
          ...
          {b.emphasis && b.x2 - b.x1 > 40 && (
            <text x={b.x1 + 6} y={14} fill={b.stroke} fontSize="10" fontWeight="600" fontFamily="system-ui">
              {b.label}
            </text>
          )}
        </g>
